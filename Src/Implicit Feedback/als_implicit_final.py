"""
als_implicit_final.py
─────────────────────────────────────────────────────────────────────────────
Implicit ALS — Final Evaluation Script
Runs two models:
...
Update RANK and REG_PARAM with best values from als_implicit_tune.py

Usage:
  spark-submit --deploy-mode client als_implicit_final.py
─────────────────────────────────────────────────────────────────────────────
"""
...
# Confidence weights (fixed from prior tuning)
ALPHA = 10
BETA  = 40
# ─────────────────────────────────────────────────────────────────────────
...
spark = SparkSession.builder \
    .appName(f"als_implicit_final_rank{RANK}_reg{REG_PARAM}") \
...
print(f"\n{'='*60}")
print(f"  ALS IMPLICIT FINAL | rank={RANK} reg={REG_PARAM} iter={MAX_ITER}")
print(f"  alpha={ALPHA} beta={BETA}")
print(f"{'='*60}\n")

# ── Helper: add confidence column ─────────────────────────────────────────
...
train_raw = add_confidence(
    spark.read.parquet(f"{BASE}/train.parquet"), ALPHA, BETA
).persist(StorageLevel.MEMORY_AND_DISK)
train_raw.count()
print(f"  Train (is_read=1, confidence) : {train_raw.count():,}")
...
tuning_hist    = add_confidence(spark.read.parquet(f"{BASE}/tuning_history.parquet"), ALPHA, BETA)
tuning_holdout = add_confidence(spark.read.parquet(f"{BASE}/tuning_holdout.parquet"), ALPHA, BETA)
...
# Save tuning model
tuning_model_path = f"{BASE}/models/als_implicit_model_tuning"
...
test_hist    = add_confidence(spark.read.parquet(f"{BASE}/test_history.parquet"), ALPHA, BETA)
test_holdout = add_confidence(spark.read.parquet(f"{BASE}/test_holdout.parquet"), ALPHA, BETA)
...
# Save test model
test_model_path = f"{BASE}/models/als_implicit_model_test"
...
results.coalesce(1).write.mode("overwrite").csv(
    f"{BASE}/results/als_implicit_final", header=True
)

# ── Final summary ─────────────────────────────────────────────────────────
print(f"\n{'='*60}")
print(f"  FINAL RESULTS — IMPLICIT ALS")
print(f"  rank={RANK} | reg={REG_PARAM} | alpha={ALPHA} | beta={BETA} | maxIter={MAX_ITER}")
print(f"{'='*60}")
...
print(f"  Results saved   : {BASE}/results/als_implicit_final")
print(f"{'='*60}\n")
